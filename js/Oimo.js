...
var OIMO = {
  ...
  lerp(a, b, percent) { return a + (b - a) * percent; },
  rand(a, b) { return OIMO.lerp(a, b, OIMO.random()); },
  randInt(a, b, n) { return OIMO.lerp(a, b, OIMO.random()).toFixed(n || 0) * 1; },

  int(x) { return ~~x; },
  fix(x, n) { return x.toFixed(n || 3, 10); },

  clamp(value, min, max) { return OIMO.max(min, OIMO.min(max, value)); },
...
OIMO.RigidBody.prototype = {
  ...
  rotationAxisToQuad(rad, ax, ay, az) { // in rad
    ian;
...
OIMO.Mat33.prototype = {
  ...
  mulRotate(m, rad, ax, ay, az, Prepend) {
    var prepend = Prepend || false;
    var s = OIMO.sin(rad);
    var c = OIMO.cos(rad);
    var c1 = 1 - c;
...
OIMO.ContactManifold.prototype = {
  ...
  reset(shape1, shape2) {
    ...
    * @param   flip
    */
  addPoint(x, y, z, normalX, normalY, normalZ, penetration, flip) {
